...
| 10 | **`precision="auto"`'s gate is a CONDITION-NUMBER heuristic, not a proof** (doc 32). It declines fp16-fragile fns, unsafe pow, data branches, image comparisons, out-of-range literals, and image-lineage amplification — a flow-sensitive gain+magnitude analysis (`precision_policy._amplification_hazard`) catching chained/squaring/fan-in (dot/matrix/length/cross)/round-trip/builtin-dimension/`fit`/array amplification, with scalar const-propagation. Verified **0 accuracy violations across 225 adversarial programs (2 red-team rounds) + the fuzzer**, headline preserved. It is STRONG, not COMPLETE — it over-declines rather than risk accuracy, and a per-cook finiteness net re-cooks fp32 on any non-finite (never memoized — doc 32 C2). auto is **experimental + ~perf-neutral** (the net costs ~what fp16 saves); the raw fp16 win is expert `precision="fp16"`. Any *accepted* program exceeding 3.9e-3 is a gate bug. | `test_c1_amplification_gate` + `test_c2_data_dependent_nan` + gate-filtered fuzzer (PR-LP2) |
