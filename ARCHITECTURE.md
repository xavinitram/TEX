...
source text
  ...
  → EXECUTION TIER (one of):
       ...
       codegen      (tex_runtime/codegen.py)        AST → flat Python fn (no torch.compile)
                    (+ tex_runtime/codegen_masked.py  the emit-time MIRROR of masked_flow —
                     a second statement-emitter table, bound per compile only for a flagged
                     program, so an unflagged one emits byte-identical source)
...
state machine (and exposes `cook_ms` medians to the scheduler). **Scheduling**
(`tex_cookqueue.py`, v0.31 SCHED-4/PRED-1) is the two-tier preemptive cook queue: three
admission classes, cooperative preemption through the existing SCHED-3 yield points, and a
speculative admission policy priced by **profiling** (`tex_runtime/profile.py`, PROF-1 — an
...
(NEG-2 added `tex_chain`, and the count is **still 2** for the same reason: it imports
...
the compile blacklist) to `warm_state.json` so a relaunch skips re-trialling; and CACHE-4 splits
