...
- **PM-11** (v0.40.1, **shipped**): the fused viewer transform (`viewer_exposure()`/
  `viewer_gamma()`) cooks inside the same program as the comp — the proof that COLOR-1 landed as a
  codegen seam rather than as a linked library. CUDA-graph capture of it is not yet supported;
  pencilled to v0.40.2.
...
| v0.40.2 | **Close the register, linear-light edition** — the round's own leftovers, closed | `TRK-162` (an ARRAY-typed `if`/`for` condition type-checks clean under `_check_scalar_condition` — which rejects `is_vector` but not `is_array` — then crashes at cook with a bare `RuntimeError`; engine-profile-only, `array_wires`, never reachable from ComfyUI's default profile; fix: reject `is_array` too, with a named diagnostic), `COLOR-1-R1`/`TRK-163` (a `[N,N,N,3]` LUT bound beside an image whose H or B coincidentally equals N is not recognised as heterogeneous by `shared_tile_height`/`shared_batch_size`, so `run_tiled`/`run_batch_strips` could slice the LUT as an image; not LUT-specific, confirmed not fixed by COLOR-1's own follow-up; fix: route the `non_spatial_args` exclusion into the co-tiling check), **viewer builtins as CUDA-graph static inputs** (PM-11's own reopen gate: feed `viewer_exposure`/`viewer_gamma` as two more static per-replay input buffers beside `bindings`, the same mechanism already named for the playhead — capture once, replay twice at two different `viewer_context` values, assert the two replays differ and neither recaptures) | — |
