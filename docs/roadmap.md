...
| v0.34.0 | **Sources & sinks** — I/O joins the pipeline (doc 39 §v0.34.0) | DATA-7 (FrameProvider: the host source protocol, `fetch_time`/`sample_time`, footprint-registered and LRU'd — the seam async file I/O plugs into), IO-1 (L, design doc: bindings may arrive as PROMISES and the cook queue admits a branch when ITS inputs land, not when the frame's do; plus a prefetch API the host drives with a predicted frame window on Tier-B time — TEX owns readiness semantics and the provider contract, decoding stays host-side), async writes (S, contract only: a completed frame is an XPU-2 `FrameHandle`, the host writes on its own thread and drops the reference; TEX's obligation ends at "the next cook starts immediately" — a test, not a subsystem), PM-9 | `tex_provider.py` (pencil) |
