...
- **PM-7** ✅ (v0.31): a preempted interactive cook reaches the head of the queue and
  the speculative tier never delays it — measured, not argued.
...
- **PM-10** (v0.37.0, **met**): a multi-layer EXR round-trips losslessly — `tex_io.exr`
  `read_layers` → `write_layers` → `read_layers` is equal per plane, bitwise, for a two-layer
  plus bare-`Z` file (`tests/test_v037_exr_layers.py`, fixtures written at runtime) — and a
...
- **PM-11** (v0.40.1, **shipped**; CUDA-graph capture of it **shipped v0.40.2**): the fused viewer
  transform (`viewer_exposure()`/`viewer_gamma()`) cooks inside the same program as the comp — the
  proof that COLOR-1 landed as a codegen seam rather than as a linked library — and now captures and
  replays without recompiling on a tweak, the same tier a fused viewer stage under active scrubbing
  most wants.
...
| v0.33.0 | **Remember more** — the memory tiers get deep (doc 39 §3) | PREC-1 (the deferred fp16-*storage* decision, argued fresh and shipped opt-in per `put`; colour/data split SHIPPED at M-3's output-kind seam (`choose_storage(kind=)`: IMAGE packs, MASK/LATENT never), plus an explicit pin and a range gate; viral preview-ness pinned), CACHE-8 (VRAM→RAM→disk residency with promote-on-reuse, 13.2-13.8x/13.7-18.7x measured over two runs; uint16 offered not chosen; ~~entropy codec~~ **measured and rejected**; ~~GPUDirect~~ no-go), XPU-2 (`FrameHandle` + fence, engine-only by assertion; ~~async demote~~ deliberately synchronous — pinning is for transient staging) | `tex_packing.py`, `tex_runtime/streams.py`, `docs/preview-tier-precision.md`, `docs/compressed-cache-tiers.md`, `docs/async-egress.md` |
| v0.33.1 | **Mind the tiers** — the v0.33.0 release audit's findings, closed | 8 frame-cache fixes, all behind armed paths: doubly-demoted frames driving `_bytes_by_dev` negative; `get()` serving storage dtype through a second lock acquisition; the P0-6 reattach residual (and its decorative pin); uint16 frames destroyed by the spill tier; `clear()` losing to an in-flight spill; an unlocked stale-epoch cleanup; a dead MRU guard; fp64 sources. Plus the `.frame` format-version field and the missing P0-5 tap-remap pin | `tests/test_v0331_audit.py` |
| v0.33.2 | **Mind the tiers, again** — the v0.33.1 audit's findings, closed | Per-key spill ORDERING (a replaced frame's older write could land last and the disk tier served stale pixels forever); a restore racing `clear(disk=True)` RESURRECTING the cleared frame; `clear`'s tail asserting a definite-empty index over a frame spilled during its unlocked unlink walk; `quality` promoted to an entry slot and a `.frame` field (fmt 1→2) so `patch_region` cannot launder a preview base; five closures (bare-string `propagate_quality`, a future `.frame` format decoded rather than refused, a shape mismatch raising out of a refusing API, `_promote` copying under a composite's lock, disarm not cancelling queued demotions). Plus the P0-4a host-pattern migration and the all-miss prologue measured rather than asserted | `tests/test_v0332_audit.py`, `benchmarks/allmiss_prologue_bench.py` |
| v0.34.0 | **Sources & sinks** — I/O joins the pipeline (doc 41 §3) | DATA-7 (FrameProvider + `fetch_time`/`sample_time` + a playhead-aware governed media pool at `evict_order=40`; footprint `'image'` not `('frame',i)` — ROI-6 reads that argument as a *batch* index and a source time is not one), IO-1 (`Promise` bindings with declared identity, the CookQueue `WAITING` state, `declare_window` prefetch with a structural PROF-1 pollution guard; `infer_binding_type` now REFUSES unknown values as E7005 instead of typing them FLOAT), async writes (S, contract only — shipped as a test; `tex_cli` + `host_demo` adopt the handle), R2-archive (compat goldens become an append-only per-version archive; `regen()` → `freeze()`), ~~FUS-cap segmenting~~ **rejected with the number** (12.9× peak memory, 1.54× slower on CPU at 1024²; the cliff is protective), PM-9 ✅ **94.3% of I/O hidden** | `tex_provider.py`, `docs/frame-providers.md`, `docs/async-io.md`, `docs/fusion-cap-decision.md` |
| v0.35.0 | **Common ground** — one cook grid, two native builtins, a checkpoint gate that says why it refuses, and the decisions Planes stands on | Phase 0, the carry-forward register closed: CF-6 (the cook grid becomes the broadcast consensus of the bindings a program *reads*, derived once by `_consensus_extent` where six first-wins copies lived), CF-1 (residency half; the latency half stays open with its argument written), CF-2 (`RoiComp` on `tex_roi.chain_windows`; a whole-frame partial recook checks its prefix), CF-3 (re-gated on a measurement), CF-4 (`requalify`/`preview_entries`), CF-5a/5b, CF-7 (pins for facts synchronised only by comment); ROTO-lang **decided on a measurement — not shipped**. Phase 1: DATA-6's design doc + a standing compat-scan tripwire. Then HOOK-3 (`tex_fusion.collapse_linear` + `tex_checkpoint.gate_refusal`), ASK-1 (`convolve`), ASK-13 (`patch_dist`). Planes moved to v0.36.0 and every later row one place | `docs/roto-lang-decision.md`, `docs/plane-bindings.md`, `tools/planes_compat_scan.py` |
...
| v0.39.0 | **Every pixel its own way** — masked per-pixel control flow, language `0.25` | Language `0.25` **shipped**, one release early against the pencil below (LANG-1…LANG-L7): the `//!tex` pragma carries on the compiled program; `tex_api.flow_plan` is one structural walk both tiers and both satellite consumers trust; a per-pixel `break`/`continue`/`return` and a per-pixel loop bound now have a masked meaning on both the interpreter (`tex_runtime/masked_flow.py`, proved against an independent scalar oracle) and codegen (`tex_runtime/codegen_masked.py`, bitwise-equal to the interpreter over the corpus and 40 fuzz-generated adversarial programs); the CUDA-graph capture gate and the fp16 auto-precision decision each ask the same plan a second question rather than growing a parallel analysis. `tex_api.LANGUAGE_VERSION` moves `0.24` → `0.25` (compat freeze #3, `0.25.json`, 141 goldens; `0.23.json`/`0.24.json` unchanged), and `W7007`/`W7008`'s loop half become conditional on the program's effective level. One breaking change ships ungated on the pragma: `E3015` (a `break`/`continue` at a function's top level, defined inside a loop, now refuses at every language level — closes `TRK-28`, measured reach zero). Folded in opportunistically, not itself pencilled: `NEG-6` (the results-cache key-minting leaf moves to `tex_results_keys.py`, floor 1958→1887), `CACHE-11` (`ResultCache.touch_promote`, the narrowed residency hint), `LIB-1` (`tex_runtime/stdlib.py` splits into a facade over eight per-domain leaves), `CG-1`/`CG-2` (a codegen cross-process reproducibility fix, an epoch-coverage gap, a `0.23` scatter-write guard fix, and a fuzz-generator channel-width fix), `GATE-1` (six ratchet-test causes repaired). Every cache tier goes cold once on adoption. **Linear light (COLOR-1), pencilled here, moves to v0.40.0 — the two rows simply swap, since this release consumed v0.40.0's pencilled content one version early rather than inserting new content, so nothing after v0.40.0 moves** (unlike the v0.35.0/v0.36.0/v0.38.0 Planes/Linear-light shifts, each of which *did* displace every later row by one) | `tex_results_keys.py`, `tex_runtime/masked_flow.py`, `tex_runtime/codegen_masked.py`, `tex_runtime/stdlib_core.py` + 7 domain leaves, `docs/masked-control-flow.md` |
| v0.40.0 | **Linear light** — colour becomes a language citizen | COLOR-1 **shipped**, lanes A/B/D/E (lane C, the fused viewer transform / PM-11, deferred — see v0.40.1): five newly reserved builtins `rec709_to_linear`/`linear_to_rec709` (BT.709 EOTF/OETF), `acescg_to_linear`/`linear_to_acescg` (a fixed 3×3 AP1↔linear-Rec.709 primary change), `apply_lut3d(rgb, lut)` (trilinear 3D LUT lookup, `tex_io/lut.py`'s new `.cube`/`.spi1d` reader); `tex_marshalling.COLORSPACES` grows `"rec709"`/`"acescg"`; the stdlib registry grows a declared `non_spatial_args` field (any function can now exempt an argument from the whole-image cook-grid consensus, replacing the point release's one-off LUT memo). **A second, ungated breaking change**: `TRK-9` — a hand-authored DAG manifest with an unanchored generator stage (no `image_input`, no chain/source/feed) is now refused at validate time on both the feeds-DAG and no-feeds paths; it previously cooked fused with no stage-by-stage meaning and raised a bare `RuntimeError` on reconstruction. Ten TRK rows closed opportunistically as docs/tests hygiene, no behaviour change (`TRK-10`, `TRK-11`, `TRK-12`, `TRK-90`, `TRK-97`, `TRK-107`, `TRK-110`, `TRK-120`, `TRK-140`, `TRK-157`), plus two correctness fixes found while cutting the release (`TRK-6`: `sample_mip`'s fp16 grid buffer now reconciles to fp32; `TRK-115`: a `[B,H,W,1]` scalar-field binding now normalises at the point of use, invariant-7-safe) and two codegen-tier fixes (`TRK-141`: a codegen-internal control-flow escape reaching a compile tier's catch is now named and recorded, not swallowed as an ordinary decline; `TRK-142`: a user function's own parameter is no longer fast-pathed as scalar in codegen). `CACHE-8`'s rejected-decision entry re-measured with blosc2 (`BRIEF-8`, the embedding host's own ask): compression stays the embedding host's, never TEX's own cache tiers. `LANGUAGE_VERSION` stays `0.25`; no compat freeze owed. Two new findings opened and pencilled to v0.40.2 (below): an ARRAY-typed `if`/`for` condition that type-checks clean and crashes at cook, and a LUT/image co-tiling shape collision (`COLOR-1-R1`) | `tex_io/lut.py` |
| v0.40.1 | **Every path sees the light** — the fused viewer transform | PM-11 **shipped, REMOVED in v0.42.0** (no consumer in ComfyUI or the embedding host) (COLOR-1 lane C, deferred from v0.40.0): two reserved zero-arg builtins `viewer_exposure()`/`viewer_gamma()`, mirroring `frame`/`fps`/`time`'s shape but never declined by codegen (interp/codegen agree bit-exactly); `viewer_context=` on `tex_engine.cook`/`prepare` threaded through every tier strategy, the five `tex_memory.py` memory-pressure functions, both OOM-ladder rungs, and the fused-chain/checkpoint family (`tex_chain.py`, `tex_checkpoint.py`) — every route PM-11's first cut left unreached; the compile fingerprint/`_compiled_cache` stay viewer-free on every path (no recompile), while `tex_results_keys.lineage_key` and CACHE-6/7's boundary taps now key on `viewer_context`, but only for a program that actually reads it (`interpreter._reads_host_context_cached`), so a non-viewer program's key is byte-unchanged. A declared `reads_host_context` `@stdlib` field (the `non_spatial_args` shape, generalised) replaces three hand-kept copies of the same name literal. **CUDA-graph capture of either builtin remains barred** — see v0.40.2, below | — |
| v0.40.2 | **Fail early, replay freely** — the round's own leftovers, closed | **Shipped**, exactly the three items pencilled above. `TRK-162` **fixed**: `_check_scalar_condition` now rejects `cond_type.is_array` too, raising a new diagnostic, `E3501`, at type-check time — reachable on the default ComfyUI profile (a plain local array condition, no engine-profile flag needed), and the one user-visible behaviour change this release makes: a program that used to crash at cook now fails to compile instead. `COLOR-1-R1`/`TRK-163` **fixed**: `_shared_dim_size` (backing `shared_tile_height`/`shared_batch_size`) now takes the `non_spatial_args`-derived exclusion set, so `run_tiled`/`run_batch_strips` pass an excluded binding (an `apply_lut3d` LUT) through whole instead of slicing it by coincidental shape match; `shared_tile_width`/`run_tiled_halo` carry the same structural shape but are not exploitable today, noted for symmetry. **Viewer builtins as CUDA-graph static inputs, shipped, REMOVED in v0.42.0** (the mechanism existed only to make the two v0.42.0-removed builtins capturable, and is dead by construction with them gone): `GraphedProgram` owned one persistent buffer per host-context builtin a captured program called, seeded at capture and `fill_()`ed fresh before every `.replay()` — the same mechanism `bindings` already uses; `viewer_context` never entered the capture key, so one captured graph served every tweak. `frame`/`fps`/`time` remain barred (no host playhead to feed yet). A declared `host_context_default` field on the registry replaces three hand-kept `1.0` literals. No reserved name, no `LANGUAGE_VERSION` move, no default moved, no new module filename | — |
| v0.40.3 | **Close the ledger** — six tracker rows and a citation-budget correction, plus five benchmark-tooling rows | **Shipped.** `TRK-116` (the PLANES arm of `_check_channel_access` returns like its string/matrix/array siblings — diagnostic shape only, confirmed ComfyUI-invisible), `TRK-144` (`tex_tool`'s duplicate `_ver_tuple` folded into `tex_api`'s one, more general definition), `TRK-154` (a masked user-function call site named on `FlowPlan`, so the CUDA-graph capture gate declines up front instead of capturing into a stream-capture error), `TRK-165` (the TRK-163 non-spatial exclusion threaded into `shared_tile_width` and `tex_tiling.py`'s two planners — symmetry only, unexploited today), `TRK-164`/`TRK-108`/`TRK-109` (two stale doc citations re-pointed, `effort-based-checkpoints.md` §7 rewritten for CACHE-7's thread-safety fix, and the CHANGELOG's own dangling citation named — citation-warning budget 24→21 across the round). Five tooling-only rows (`TRK-81`, `TRK-89`, `TRK-100`, `TRK-126`, `TRK-133`) touch only `benchmarks/`/`tests/`, no `tex_*` production module. No reserved name, no `LANGUAGE_VERSION` move, no default moved, no new module filename. Two new advisory findings opened, unexploited, not pencilled to any release: the tiling anchor-selection loops still pick a spatial binding by shape rather than by name, and `tools/gate.py`'s counts leg can't see a shape-mismatch refusal. **Next up, already pencilled: v0.41.0 "The library compiles"** — `TOOL-6`/`TOOL-7`/tool manifest polish; nothing here changes that pencil | — |

| v0.41.0 | **Fewer round trips** — the counts round's open debt, paid down (SHIPPED) | `TRK-166`/`66`/`67`/`68`/`71`/`73`/`83` (host-scalar readbacks and fixed-pipeline calls removed from the interactive path), `TRK-69` (the one pixel-visible fix — `bilateral_filter` fp32-rounds a bare float sigma like `gauss_blur`, direct callers only), `TRK-95`/`16`/`103` (correctness/hygiene fixes), `TRK-80` data half + `TRK-131`/`159` (tooling/docs). See `CHANGELOG.md` | — |
| v0.42.0 | **The interactive floor** — per-cook fixed overhead on the interpreter scrub path, plus host-facing audits (SHIPPED) | `interp_chain_scrub` (new `BENCH-2` scenario), `TRK-65` (region-independence proved on the unfolded source, short-circuiting most folded re-checks), `TRK-84` (bit-exact coordinate-ramp cache, `pan` −4 kernels/−4 allocs/tick), `prewarm(cancel=)`, a structured OOM refusal (`EngineRefusal(code="out-of-memory")`), a public `.code` on the four diagnostic exception classes, a `ResultCache.put(kind=None)` footgun warning, a cold-import ratchet. **`viewer_exposure`/`viewer_gamma`/`viewer_context=` and the CUDA-graph host-context buffers (PM-11, v0.40.1; v042-graph, v0.40.2) are REMOVED outright** — no consumer in ComfyUI or the embedding host; both names un-reserved. See `CHANGELOG.md` | — |
| v0.42.1 | **Paid once, not per cook** — a repeat-measurement patch (SHIPPED) | A reference-GPU sitting record found the interpreter's cold first cook regressed since v0.40.0: a per-cook AST walk for non-spatial-argument bindings, and the registry rescan that walk depended on, both re-paid on every cold compile instead of once. The registry lookup is now cached; the AST walk is skipped when the program's own source text names no non-spatial-argument builtin (sound — TEX has no string-built calls), and a fused chain's interpreter fallback always takes the safe walking path. Cold first cook: 0.977 → 0.999 of v0.39.0's baseline, measured twice. No `_AST_EPOCH` move, no cache goes cold. See `CHANGELOG.md` | — |
| v0.42.2 | **Fenced, not flaky** — a hardening patch (SHIPPED) | A CUDA event fence on `ResultCache`'s restore leg for a caller reading from its own stream (not reproduced through `ResultCache` itself, 0/2080 stress trials — closes a demonstrated-in-isolation gap); two CodeQL-flagged backtracking regexes fixed (the gate's own summary parser, a test's import-lint pattern); the invariant-#7 canary's vacuous match fixed plus one named allow-list entry; wall-clock assertions moved behind a `timing` marker and out of the fast tier; the fast tier now refuses a shared git stash. See `CHANGELOG.md` | — |
| v0.43.0 | **Nothing unannounced** — tools grow part of their version story (SHIPPED, partial) | TOOL-7 **shipped**: install-time prewarm (`install_tool(warm=True)`) is now cancellable via the same `CancelToken` contract `tex_api.prewarm` already has, plus a read-only `tool_warm_status` query — CACHE-3 mechanics, never shipped bytecode, and CONSENT-GATED because TOOL-5's shipped default stays validate-only at install. Two riders **shipped**: a read-only `graphed.capture_pending` CUDA-graph-capture-pending query, and a mechanical merge of the interpreter's and the compiled tier's near-identical H2D-ingest-event fences into one helper (the codegen and verdict caches go cold once on this upgrade). A new eighth cheap-tier ratchet lints every pushed file against citing a local-only path or naming the embedding host. **TOOL-6 (side-by-side installed versions, semver-range resolution, per-project pinning, PM-4's compat-corpus floor) and the manifest-schema polish that would carry it are HELD** — designed but not built, an open question still with the author; carried forward unscheduled, not displacing v0.44.0's pencil below. **Corrects the `v0.42.2`-pencilled rider**: the shared CUDA-fence primitive floated there (merging `ResultCache`'s restore fence with `FrameHandle`'s egress fence) was examined at design time and explicitly declined — three fence shapes, not one duplicated pattern, per `DEVELOPMENT.md`'s rejected-decisions register; the interpreter/compiled ingest-fence merge above shipped in its place. See `CHANGELOG.md` | — |
| v0.43.1 | **Every code tested** — an error-code coverage patch (SHIPPED) | Test-only, no product module touched. The "tested" error-code ratchet no longer counts a comment/docstring mention as a real trigger; nine previously-untested codes gained real trigger tests (four confirmed genuinely defensive, pinned against a live program each); the corpus test harness's post-optimize type map now mirrors production's re-check exactly (zero goldens moved); a pinned test-suite contract (`tests/helpers.py`'s `__all__`) is now documented where a contributor would look. A routine registry security-scan review found every current finding inherent, none code-fixable. No `LANGUAGE_VERSION` move, no default moved, no new module filename. See `CHANGELOG.md` | — |
| v0.43.2 | **The stream comes back** — a CUDA-graph capture-failure stream leak, fixed (SHIPPED) | A failed CUDA-graph capture could leave the calling thread's current CUDA stream permanently non-default (PyTorch's own `torch.cuda.graph()` skips its stream restore when ending the capture itself raises); later work on that thread then ran cross-stream with no fence, which is what actually produced the `wrong=2` threaded-cache race `TRK-178` had hardened a DIFFERENT, unreached code path against. `GraphedProgram.capture()`/`_recover_from_capture_failure` now save and unconditionally restore the caller's stream on every exit path; a permanent autouse test guard resets and fails any test that leaves the current stream non-default. ComfyUI's single-threaded default-stream path is unaffected — the exposure is any caller whose capture fails and that keeps working on the same thread. No `LANGUAGE_VERSION` move, no default moved, no new module filename; the tier-verdict cache goes cold once (`tex_runtime/graphed.py`), `.pkl`/`.cg` stay warm. See `CHANGELOG.md` | — |
| v0.44.0 | **The interactive path, measured** — two mechanical splits, a real tick shape, cancellation (SHIPPED) | Two mechanical splits regain size headroom (`tex_results_residency.py` off `tex_results.py`; `interpreter_spatial.py`/`interpreter_control_flow.py`/`interpreter_binding.py` off `tex_runtime/interpreter.py`, all mixins, byte-identical). The embedding host corrected the interactive path's own shape (whole-frame, `roi=None`, `compile_mode="none"`, unfused, fp32, `use_cache=False`) — two new `BENCH-2` counts scenarios (`whole_frame_chain_d1`/`_d3`) measure it directly; a `checkpoint_serve` tick is ~11.5% faster (a source-hash memo), the ROI scrub scenario shows no measurable change. Cancellation now reaches the codegen stencil route and polls between a multi-pass builtin's own passes; `CookCancelled` is no longer mis-wrapped as a generic interpreter error. The release gate reds a leg that dies before writing a report and refuses up front on an unimportable checkout. TOOL-6 (v0.43's HELD item) is DECLINED on ownership — the host's own versioned store, resolution and trust hash cover it; TEX must never write a `content_hash` into a manifest by default (`DEVELOPMENT.md`'s rejected-decisions register). No `LANGUAGE_VERSION` move, no default moved. **Found and fixed before shipping: the three new interpreter mixin files are now `_CODEGEN_FILES` members too, beside `interpreter.py`** (`tex_results.py` is on no watch-list, so its own new sibling needs none); a derivation test now guards the whole class. **A noise-kernel load failure (e.g. Windows Application Control blocking a freshly-compiled kernel) no longer fails the cook** — it now falls back to the pixel-identical eager tier and warns once, closing a real user-facing gap (`TRK-182`). See `CHANGELOG.md` | v0.43.2, until v0.45 |
| v0.45.0 | **The adoption point** — a frozen public seam, two small asks, a persisted-state audit (SHIPPED) | Answers the embedding host's 2026-09-25 asks in full: (1) `ResultCache.spill(key) -> bool` — spill one named entry to the disk tier, mechanism not policy; (2) `Program.time_reads: frozenset[str]` — which of frame/fps/time/fetch_time/sample_time a program reads, derived once at compile time; (3) the seam freeze test pins 116 product symbols (names AND keywords) plus 87 test-only ones, per the host's own census; (4) every internal the census named now has a tier row or a named public replacement in DEVELOPMENT.md; (5) a PURITY contract for `lineage_key`/`boundary_lineage_key` (pure given their arguments plus a per-process `env_epoch()`); (6) SECURITY.md gets a full persisted-state and deserialize-site inventory — the pickle sites are confirmed down to one, MAC-gated; five JSON-only sites carry no integrity check, filed as an open, data-poisoning-class-only advisory. No `LANGUAGE_VERSION` move, no default moved, no new reserved name. **From this release, TEX is patch-only until the embedding host re-pins onto it and reports back** — every later minor below is HELD, not merely pencilled, until that report lands. See `CHANGELOG.md` | v0.45.0 |
| v0.45.1 | **Blocked, not broken** — a correctness patch under the patch-only regime (SHIPPED) | `v0.44.0`'s noise-kernel-load fallback caught a blocked compile only when the failure arrived as a bare `ImportError`/`OSError`; on this box's torch build it instead arrives wrapped in `torch._inductor.exc.InductorError`, which the bare check missed, so a cook could still fail. Now any kernel-load failure anywhere in the exception chain (wrapped or bare) falls back to the eager tier, same pixels, one warning; a real compile bug still surfaces. No `LANGUAGE_VERSION` move, no default moved, no new reserved name, no new module filename — fix lands entirely in `tex_runtime/noise.py` (a `_CODEGEN_FILES` member: one cache cold-start, `.pkl` stays warm). The recommended adoption point for a
