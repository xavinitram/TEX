...
- **PM-10** (v0.37, pencil): a multi-layer EXR round-trips losslessly, and a program
...
| v0.37.0 | **Planes** — the data model widens | DATA-6 (XL, design doc: named multi-plane buffers, `@src.N`/`@src.Z` at the marshalling seam, TEXType stays ≤ VEC4, `layer.channel` naming verbatim for EXR; per-plane lineage keys give channel-level cache invalidation with no new mechanism), EXR layers/parts in `tex_io` (M), PM-10 | — |
