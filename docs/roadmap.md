...
- **FUS-0 (S, shipped v0.20.1).** Fused chains cannot reach `torch_compile`/`auto` in
  ...
  `tex_engine.py:443-460`) but the node's `execute()` computed `fused_fp` only under
...
**Unversioned follow-up, not pencilled to any release:** the `time_context`/`viewer_context`
single-carrier cleanup. `ExecContext` now threads two structurally identical
`dict | None` kwargs in parallel through every tier strategy, every memory-pressure
function and the fused-chain family — `v0.40.1`'s own simplify C only unified the two
dicts' *encoding* (`tex_results_keys._canon_float_dict`), not the carrying. A single
`host_context: dict[str, dict]` (or equivalent) would halve the parameter surface every
future host-context value adds to. No release depends on it and no test pins the two-kwarg
shape as load-bearing, so it ships whenever it is convenient rather than against a theme.
| v0.41.0 | **Fewer round trips** — the counts round's open debt, paid down (SHIPPED) | `TRK-166`/`66`/`67`/`68`/`71`/`73`/`83` (host-scalar readbacks and fixed-pipeline calls removed from the interactive path), `TRK-69` (the one pixel-visible fix — `bilateral_filter` fp32-rounds a bare float sigma like `gauss_blur`, direct callers only), `TRK-95`/`16`/`103` (correctness/hygiene fixes), `TRK-80` data half + `TRK-131`/`159` (tooling/docs). See `CHANGELOG.md` and `docs/shard-changelog-0.41.md` | — |
| v0.42.0 | **The library compiles** — tools grow their version story | TOOL-6 (side-by-side installed versions, semver-range resolution, per-project pinning: `Blur@^1.2` gets the same pixels next year, with PM-4's compat corpus as the enforcement floor), TOOL-7 (install-time prewarm on the background pool persisting `.pkl`/`.cg`/warm verdicts — CACHE-3 mechanics, never shipped bytecode, and CONSENT-GATED because TOOL-5's shipped default is validate-only at install), tool manifest polish (S) | — |
| v0.43.0 | **Batch honest** — headless throughput | SCHED-5 (the throughput profile: preemption off, Tier-B admission replaced by frame-pipeline lookahead, governor switched to write-and-evict — a scheduling profile on the same engine and queue, exactly as the report insists), BATCH-1 (L, design doc: time-invariance classification from `used_builtins` plus the host's animated-param set, so a frame-invariant stage's lineage key is frame-INDEPENDENT and CACHE-2 serves it across the whole range for free; the work is the classifier API, lifetime hints, and cost-aware skip) | — |
| v0.44+ → v1.0 | **Engine era** | the remaining §4 programs (GRAPH/XPU-1/DATA-5/STOCK/ML/ROTO), each behind its own design doc | `tex_graph.py` |
