...
`tex_runtime/compiled.py::_cuda_headroom_ok` (`compiled.py:1019`), once per program, deciding
...
6. **The per-cook fixed pipeline, paid ten times on a whole-frame cook.** Per cook, at head:
   ...
   all called from `tex_engine.run` (`tex_engine.py:1281`) — plus `fingerprint`, **once**
