...
| The tap key | `tex_chain.boundary_lineage_key` (`:258`, re-exported onto `tex_engine`) | Already namespaced by the cut (`flags=['tap:s{k-1}']`), so N cuts mint N keys with no key-scheme change |
| The single-tap driver | `tex_chain.cook_fused_cached` (`:353`, re-exported onto `tex_engine`) | The gate, the `FusionError` fallback, and the "correct-but-not-incremental full cook" posture |
...
`cook_fused_cached` mints the boundary key **without** `canvas=` (`tex_chain.py:406`), so a
