...
const TEX_HELP_DATA = [
    ...
    },
    {
        ...
        entries: [
            ...
            { name: "Pixel Step", sig: "px, py", desc: "px = 1/iw, py = 1/ih: one pixel's width as a fraction of the frame. Neighbouring u/v centres are 1/(iw-1) apart, not 1/iw, so u + px is not an exact neighbor step. For exact neighbor sampling use u + k / max(iw - 1.0, 1.0), or fetch() by pixel index.", example: "vec4 right = sample(@A, u + 1.0 / max(iw - 1.0, 1.0), v);" },
    ...
    },
    {
        ...
        entries: [
            ...
            { name: "rec709_to_linear", sig: "rec709_to_linear(c) \u2192 vec", desc: "Gamma-encoded Rec.709 \u2192 linear-light (BT.709 EOTF; distinct curve from sRGB).", example: "vec3 lin = rec709_to_linear(@image.rgb);" },
            ...
            { name: "viewer_exposure", sig: "viewer_exposure() \u2192 float", desc: "The host viewer's exposure gain for THIS cook (default 1.0 = no-op). Never baked into the compile fingerprint (PM-11).", example: "@OUT = vec4(@A.rgb * viewer_exposure(), 1.0);" },
            { name: "viewer_gamma", sig: "viewer_gamma() \u2192 float", desc: "The host viewer's gamma for THIS cook (default 1.0 = no-op). Never baked into the compile fingerprint (PM-11).", example: "@OUT = vec4(pow(@A.rgb, vec3(1.0 / viewer_gamma())), 1.0);" },
    ...
    },
    {
        ...
        entries: [
            ...
            { name: "convolve", sig: "convolve(img, kernel[, normalize]) \u2192 vec", desc: "General image-kernel convolution (the kernel is flipped, not correlated). kernel is a second IMAGE/MASK binding, read whole; kernel size in [1,257]. Its channel count broadcasts (1 plane -> every image channel) or weights per channel (== image channels, depthwise). normalize=1 (default) divides by the per-channel kernel sum; 0 returns the raw weighted sum. Replicate border padding.", example: "@OUT = convolve(@A, @kernel);" },
            { name: "patch_dist", sig: "patch_dist(img, dx, dy, radius) \u2192 float", desc: "Mean squared difference between the patch at this pixel and the patch at (dx, dy) pixels away. The non-local-means core.", example: "float d = patch_dist(@A.rgb, 3, -2, 1);" },
    ...
    },
    {
        ...
        entries: [
            ...
            { name: "voronoi", sig: "voronoi(x, y) \u2192 float", desc: "Alias of worley_f1 \u2014 distance to the nearest feature point. For a per-cell value use worley_id.", example: "float d = voronoi(u * 8.0, v * 8.0);" },
            { name: "worley_id", sig: "worley_id(x, y) \u2192 float", desc: "Worley cell id: a stable value in [0, 1] per cell of worley_f1's nearest feature point.", example: "float id = worley_id(u * 8.0, v * 8.0);" },
