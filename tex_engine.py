...
**SPLIT-E (v0.46) — the tier strategies follow, for the same reason.** This file had grown
to 1393 lines against its 1400 floor, 7 lines of room. The STR-2 domain — `select_tier` (pure
selection) plus the four `_run_*` strategies that actually execute one, the shared
`_interp_fallback` recovery path, `_record_codegen_defect_fallback`, and the `_run_tier`
dispatcher — moved to `tex_engine_tiers`, bodies verbatim with ONE deliberate exception:
the ROUTE-45 audit named the exact failure mode a plain mechanical move would create here.
`_get_interpreter` (tex_chain), `_tile_plan`/`_halo_tile_plan`/`_scalar_params` (tex_tiling)
— and, just as much, `execute_compiled`/
`_codegen_only_execute`/`should_stencil_route` (tex_runtime.compiled) and `CookCancelled`/
`_cancel_check` (tex_runtime.host) — were all reachable, before this split, as a name the
MOVING code resolved through THIS module's own namespace; a test (`test_trk141_codegen_
defect_fallback.py`) already monkeypatches `tex_engine.execute_compiled` for exactly that
reason. Importing any of them straight from their true source module into `tex_engine_tiers`
would silently drop that visibility for the moved caller, so `tex_engine_tiers` instead
holds a deferred reference to this module (`_tex_engine`) and reads `_tex_engine.NAME` at
every one of those call sites — see that module's docstring for the full account and why a
function-local import (`run_auto`, `run_graphed`, `tier_trace`, the `tex_memory` leaves)
needs no such treatment. `prepare`, `run`, `_dispatch_tier` and `_fp16_finiteness_net` stay
here unmoved and call the re-exported names below exactly as before, so nothing about
THEIR resolution changes. The move took this file to 1144, so the
floor goes 1400 -> 1300 (REG-2), a 156-line margin, wider than the 7 lines of room this
release started with: a floor sitting seven lines from the wall is the situation ENG-14 was
created to prevent.
"""
...
# their own modules. The move is mechanical — no body changed — and the names are
# re-bound here so that every `tex_engine.NAME` reader and every caller that stayed
# behind is unchanged. Same posture as the v0.22 lift; pinned by
# test_v022_phase1.test_eng1_node_is_a_marshaller.
from .tex_buffers import (                        # noqa: F401  (re-export)
    _owned_copy, to_dlpack, from_dlpack, is_frozen, frame_version,
    verify_unmutated, frozen_copy, freeze, _disown_inputs,
)
from .tex_tiling import (                         # noqa: F401  (re-export)
    _TDR_BUDGET_MS, _tile_plan, _tdr_strip_floor, _scalar_params,
    _halo_tile_plan, _preflight_memory,
)
# ── NEG-2: the chain cook, re-exported ──────────────────────────────────────
# The CACHE-6 stage-list family and CACHE-1's lineage keys now live in tex_chain, and
# the ENG-4 raiser + the ENG-9 interpreter pool travelled with them so that module stays
# a leaf. The move is mechanical — no body changed — and the names are re-bound here so
# that every `tex_engine.NAME` reader and every caller that stayed behind is unchanged.
# Same posture as ENG-14 above; pinned by test_v022_phase1.test_eng1_node_is_a_marshaller.
from .tex_chain import (                          # noqa: F401  (re-export)
    _compile_or_raise, _interp_pool, _get_interpreter, _clear_all_interpreter_caches,
    _compute_lineage, cook_stage_list, _is_tensor_binding, _binding_shape,
    boundary_lineage_key, cook_fused_cached,
)
# ── SPLIT-E: tier selection + execution, re-exported ─────────────────────────
# The STR-2 tier strategies (select_tier + the four `_run_*` executors, the shared
# interpreter-fallback recovery path, and the `_run_tier` dispatcher) now live in
# `tex_engine_tiers`. The move is mechanical — no body changed — except that every call
# this code makes into a name it used to reach through THIS module's own namespace
# (`_get_interpreter`/`_tile_plan`/`_halo_tile_plan`/`_scalar_params`, and
# `execute_compiled`/`_codegen_only_execute`/`should_stencil_route`/`CookCancelled`/
# `_cancel_check`) now routes through this module's own attribute instead of a direct
# import (ROUTE-45: see that module's docstring) — so every re-export below, both the
# ones this row adds and the ones ENG-14/NEG-2 already added above, is still exactly what
# a wrap on `tex_engine.NAME` sees. The names are re-bound here so that every
# `tex_engine.NAME` reader and every caller that stayed behind (`prepare`, `run`,
# `_dispatch_tier`, `_fp16_finiteness_net`, all below, unmoved) is unchanged. Same posture
# as ENG-14/NEG-2 above; pinned by test_v022_phase1.test_eng1_node_is_a_marshaller.
from .tex_engine_tiers import (                   # noqa: F401  (re-export)
