...
from .tex_results_residency import _dev_bucket, _ResultCacheResidency
...
class ResultCache(_ResultCacheResidency):
    ...
    **ONE LIVE INSTANCE PER SPILL DIRECTORY.** Thread-safety here is process-and-instance
    ...
    SPLIT-R: the CACHE-8 residency ladder (`set_vram_budget`/`_enforce_residency`/
    `_queue_demotions`/`_drain_demotes`/`_promote`) is inherited from `_ResultCacheResidency`
    (`tex_results_residency.py`) rather than defined here. It reads and writes this class's own
    `_lock`/`_ram`/`_bytes_by_dev`/`_pending_demotes`/`_demoting`/`_vram_budget` exactly as
    before the split; the mixin owns no state of its own."""
    ...
    # ── CACHE-8: residency (VRAM -> host RAM -> disk) — moved to tex_results_residency.py
    # (SPLIT-R); ResultCache inherits _ResultCacheResidency, so set_vram_budget/
    # _enforce_residency/_queue_demotions/_drain_demotes/_promote are still ResultCache
    # methods, unchanged in behaviour and lock discipline. ──
