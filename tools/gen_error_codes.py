...
_FAMILIES = [
    ...
    ("W7", "Warnings", "Non-fatal advisories (LANG-2). The program still compiles and "
     ...
     "that shadows a built-in or an outer-scope variable. W7005, W7006, W7007 and W7008 come "
     "only from analyses a host calls beside `check()` (`tex_api.color_advisories`: a "
     "gamma-space blur; `tex_api.control_flow_advisories`: a per-pixel `if` or `?:` that "
     "gathers in a branch, control flow that acts on every pixel, or control flow whose "
     "result depends on which region is cooked, so the engine declines to split the cook), "
