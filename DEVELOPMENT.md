...
Under `0.23`/`0.24` rules, a `break`, `continue` or `return` inside a per-pixel branch raises past the merge, so it acts on every pixel and leaves that branch's assignments unmerged. Under `//!tex 0.25` (masked per-pixel control flow, `docs/masked-control-flow.md`) it instead clears/records only the pixels live at that branch — `break` for the rest of the loop, `continue` for the rest of the pass, `return` recorded then cleared — and keeps evaluating for the rest (`tex_runtime/masked_flow.py`). `LANGUAGE.md` §7.1 states both rule sets for program authors; `tex_api.control_flow_advisories` (W7006/W7007) flags the older behaviour, opt-in, never from `check()` — W7007 is conditional since `0.25`: it does not fire for a program actually cooked under masked rules.

## Loops

For and while loops execute sequentially -- each iteration runs the body as vectorized tensor operations. The loop variable is a scalar (not a per-pixel tensor). A per-pixel condition (`i < n` with a per-pixel `n`) keeps the loop running while any pixel's condition holds; under `0.23`/`0.24` rules the body is not masked, so every pixel runs to the frame's maximum; under `//!tex 0.25` the loop still runs for as many passes as the region's maximum (it still runs while ANY pixel is live — the pass *count* does not change), but from the first masked pass onward each pixel's own value stops updating once its own condition goes false (`LANGUAGE.md` §7.1). Each iteration computes the body across all pixels simultaneously. The iteration limit is 1024 (`MAX_LOOP_ITERATIONS`); a live pixel still needing pass 1024 raises E6010 either way. Both `break` and `continue` are supported.

**Under `0.23`/`0.24` rules, a per-pixel loop bound also means the cook is never split.** "The frame's maximum" is a maximum over the *region being cooked* — `_loop_cond_true` reduces the condition with `.any()` over B, H and W — so a strip, an ROI window or a batch strip computes a different pass count than the whole frame does. **This sunsets under `//!tex 0.25`** (`tex_roi.region_dependent`'s clauses (a)/(b), keyed on `MASKED_FLOW_SINCE = (0, 25)`): a masked loop's per-pixel *values* no longer depend on which region is cooked (only the region's pass *count* still does, and that is a cost, not a wrong answer — see `docs/masked-control-flow.md` §4), so a `0.25`-rules program with a per-pixel loop bound splits again. The same holds for a string chosen per pixel, in either spelling — assigned inside a per-pixel `if`, or picked by a per-pixel `?:` — because a string has no per-pixel form, so both are resolved by a majority vote over the region's pixels; a string that arrives on a *wire* is only visible to the check when the caller supplies `binding_types`, which every engine call site now does. A per-pixel value cast STRAIGHT to a string (TRK-32) — `string(x)`, `str(x)`, or a `format()` call whose template actually fills a placeholder — is a sibling class with no condition or merge at all: `stdlib._scalar_from_tensor` has no per-pixel form either, so it falls back to the MEAN of the region's pixels, and a strip's mean differs from the whole frame's; `format("%f", x)` is excluded because `%f` is not a `{}`/`{:spec}` placeholder, so the template returns unchanged and the cast value never reaches the output. `tex_roi.region_dependent(program, binding_types, code)` names that class, reusing `tex_api._ControlFlowLint`'s "can this differ per pixel" rule set rather than deriving a second one, and the three planners that split a cook — `tex_engine._tile_plan`, `tex_roi.roi_plan` (and through it the halo strip planner, the engine's ROI arming and `stage_halo`/`chain_windows`) and `tex_roi.batch_sliceable` — all decline for it. The executors are deliberately untouched. The gate sits after each planner's pressure test, so an unpressured cook never reaches it, and the one user-visible consequence is that a pressured CUDA cook of such a program gets whole-frame-or-OOM instead of a wrong tiled picture. A `break`, `continue` or `return` under a per-pixel guard is NOT in the class: it fires on first arrival, identically in every region, so it still splits. `tex_api.control_flow_advisories` reports the class as **W7008**, opt-in, never from `check()`.
...
| **1 — Public** | `tex_runtime.cook_observer.register(cb) -> handle` / `.unregister(handle)` (OBSERVER-46) | The supported cook-observer seam: `cb(entry, thread)` fires once per EXTERNAL call at each of the six cook entry points (`tex_engine.run`/`cook`, `tex_chain.cook_stage_list`/`cook_fused_cached`/`boundary_lineage_key`, `tex_checkpoint.cook_checkpointed`) — the alternative to monkey-patching one of those names, one of which ROUTE-45 found a re-export + internal self-call already defeats. Nesting among the six collapses into the outermost call's single notification (documented in the module docstring); zero cost when nothing is registered; a raising callback is caught and warned once per process, never breaks the cook | `tests/test_observer46_cook_observer.py` |
