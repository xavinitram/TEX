...
#: `_consensus_extent`'s answer when `program.non_spatial_calls is False` — shared, not a
#: fresh `frozenset()` per cook.
...
def _consensus_extent(bindings: dict, program: Program,
    ...
    REG-1e: `program.non_spatial_calls`, set once by `tex_cache.TEXCache.compile_ast` (the
    pipeline both the normal path and `tex_fusion.compile_fused` share), says whether this
    program can bind a non-spatial argument at all. `False` skips the walk below entirely —
    correct even on a cold compile's first cook, where the fresh `Program` always misses
    `_READS_MEMO`. `True`/`None` (not computed) walk, the safe default.
    """
    ...
    if program.non_spatial_calls is False:
        read, non_spatial = None, _EMPTY_NON_SPATIAL
    else:
        # `non_spatial` (COLOR-1, v0.40): one memoized `_READS_MEMO` lookup, shared with the
        # narrowing branch's `read` via `_reads_and_non_spatial_cached`.
...
def _collect_identifiers_and_calls(program: Program) -> tuple[frozenset[str], frozenset[str]]:
    """ONE pass: (builtin identifier names, function-CALL names). `_collect_identifiers` is
    the PUBLIC accessor over the first half (its original bare-frozenset contract).

    REG-1e: the second half feeds `tex_cache.TEXCache.compile_ast`'s `Program.non_spatial_calls`
    — the SAME hand-dispatched pass `_collect_identifiers` already paid for, extended to
    also record a `FunctionCall`'s name; never a second walk, and deliberately NOT the
    generic `iter_child_nodes` walk `_collect_binding_reads_and_non_spatial` uses (that one
    must never miss a reader; this one already keeps its own dispatch in sync)."""
    found: set[str] = set()
    calls: set[str] = set()
...
def _collect_identifiers(program: Program) -> frozenset[str]:
    """Builtin variable names a program mentions (fast single-pass scan). Delegates to
    `_collect_identifiers_and_calls`; see its docstring."""
    return _collect_identifiers_and_calls(program)[0]
